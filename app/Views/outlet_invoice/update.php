...
<head>
    ...
    <title>Update Outlet Invoices</title>
...
<div class="wrap">
    ...
    <div class="container">
        ...
        <ul class="breadcrumb">
            ...
            <li class="active" >Update Outlet Invoices</li>
        </ul>

        <div class="row" style="margin-top: -20px">
            <h1 style="margin-left: 13px;">Update Outlet Invoices</h1><BR>
        </div>
        <br>
        <?php if($fetch_data){
        foreach ($fetch_data->result() as $rs) {?>
        <form id="w0" action="<?php echo base_url();?>updating_outlet_invoice/<?php echo $rs->doi_id ;?>" method="post">

            <div class="col-sm-4">
                <div class="form-group required">
                    <label class="control-label" >Invoice No.</label>
                    <input type="text" class="form-control" name="invoice_no" placeholder="Enter Invoice No." value="<?php echo $rs->invoice_no; ?>" >
                </div>
            </div>
            ...
            <div class="col-sm-4">
                ...
                <div class='input-group datepicker' id='date'>
                    <input id="date_val" name="invoice_date" type='text' class="form-control " placeholder="Select Date" value="<?php echo $rs->invoice_date; ?>"/>
                    ...
                    <select  class="form-control outlet" name="outlet" id="outlet" >
                        <?php if($fetch_outlet){
                            foreach ($fetch_outlet->result() as $row) {?>
                                ...
                                <option value="<?php echo $row->outlet_id;?>" <?php if($row->outlet_id == $rs->outlet){ echo "selected";} ?>><?php echo "$row->outlet" ;?></option>
                            <?php } } ?>
                    </select>
                        ...
                        <div  class="col-sm-12">
                            <div style="margin-left: 180px; " >
                                <div class="col-sm-6" >
                                    <div class="form-group" style="margin-top: 3px; ">
                                        ...
                                        <select  class="form-control " name="sku" id="sku" ></select>
                                        <div class="help-block"></div>
                                    </div>
                                </div>
                                <div class="col-sm-2">
                            ...
                            <div class="col-sm-10 " style="margin-left: 8%">
                                ...
                                <table id="data_table"  class="table table-striped table-bordered " STYLE="text-align: center; margin-top: 20px">
                                    ...
                                    <?php
                                    if($fetch_invoice){
                                        foreach ($fetch_invoice->result() as $rs) {
                                            ?>
                                            <tr>
                                                <td class="kv-align-center kv-align-middle"><?php echo $rs->sku;?></td>
                                                <td class="kv-align-center kv-align-middle"><?php echo $this->Outlet_Invoice_Model->get_inv_desc($rs->sku);?></td>
                                                <td class="kv-align-center kv-align-middle"><?php echo $rs->quantity;?></td>
                                                <td class="kv-align-center kv-align-middle"><i class="glyphicon glyphicon-trash" style="color: red;"></i></td>
            ...
            <div class="pull-right"  >
                <button type="submit" class="btn btn-success"><b>Update Invoices</b></button>
            </div>
        </form>
        <?php }}?>
...
<script type="text/javascript">$(function () {$('#date').datetimepicker({"format":"Y-MM-DD"});});</script>
